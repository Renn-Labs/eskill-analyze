...
## 2. Higher tiers may send your analysis to external model providers
...
- **`esat-frontier`** runs a configurable reviewer roster. Depending on `ESAT_FRONTIER_ROSTER` and
  `ESAT_FRONTIER_FLEET`, that roster may include external Codex/Grok/fleet reviewers.
...
- Run `esat-frontier` at `ESAT_FRONTIER_SENSITIVITY=high` and use a local/first-party-only
  `ESAT_FRONTIER_ROSTER` for proprietary work.
